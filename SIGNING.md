...
- **`just dev`** runs an *unsigned* debug binary that is rebuilt every time, so macOS
  can never remember an "Always Allow" for it. Coot caches each password in memory for
  the life of the process, so a dev session prompts **at most once per launch**, not
  once per query — but it cannot get to zero.
...
just setup-signing        # runs ./scripts/setup-macos-signing.sh

# 2. Build the signed app.
just app-build            # runs `bun run tauri build` in app/ui
...
`just app-build` reuses the same identity and the "Always Allow" keeps working.

> On the **first** `just app-build`, `codesign` may itself pop one "wants to sign
